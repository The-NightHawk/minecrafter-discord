...
class Player:
        ...
        if request.status_code != 200:
            ...
            for dictionary in data[1:][::-1]:
                dt_object = datetime.fromtimestamp(
                    int(dictionary['changedToAt']) / 1000)
    ...
    def skinURL(self, subURL: str):
        skinType = skinModes[subURL] if subURL in skinModes else skinModes[
            'body']
    ...
    def profile(self):
        ...
        if name_history:
            embed.add_field(name="Name History",
                            value='\n'.join(
                                [key for key, value in name_history.items()]),
                            inline=True)
            embed.add_field(name="\u200b",
                            value='\n'.join([
                                value for key, value in name_history.items()
                            ]),
                            inline=True)
    ...
    def id(self):
        embed = discord.Embed(title="UUID",
                              description=f"`{self.uuid}`\n`{self.full_uuid}`")
        embed.set_author(name=self.name, icon_url=self.skinURL('avatar'))
        return embed
    ...
    def playerhead(self):
        ...
        embed.add_field(
            ...
            value=f"`/give @p minecraft:player_head{{SkullOwner: \"{self.name}\"}}`",
            inline=False)
        return embed
...
def getPlayer(player: str):
    if len(player) == 32 or len(player) == 36:
        uuid = player
        request = requests.get(
...
async def on_message(message):
    if client.user in message.mentions:
        await message.reply("You can type `.help` for a list of all commands")
    await client.process_commands(message)


@slash.slash(name="help", description="Get some help!", options=[])
@client.command()
async def help(ctx):
    embed = discord.Embed(
        title="Help",
        description="Minecrafter is a bot which can show you a player's Profile, Skin, UUID, or Name History! \nPrefix: `.` \nMinecrafter supports Slash Commands! Type `/` and choose your command!"
    )
    embed.add_field(
        name="**`.profile <username|UUID>`**",
        value="Get the profile of the player. This includes skin, name history, and UUID.",
        inline=True)
    embed.add_field(
        name="**`.skin <username|UUID> [body|head|uv|cape|avatar]`**",
        value="Get the skin of a player. You can choose from their head, body, avatar, cape, or skin texture.",
        inline=True)
    embed.add_field(name="\u200b", value="\u200b", inline=False)
    embed.add_field(name="**`.uuid <username|UUID>`**",
                    value="Get the UUID of a player.",
                    inline=True)
    embed.add_field(name="**`.history <username|UUID>`**",
                    value="Get the name history of a player.",
                    inline=True)
    embed.add_field(name="**`.playerhead <username|UUID>`**",
                    value="Get the head of a player as a Minecraft item",
                    inline=True)
    embed.add_field(name="\u200b", value="\u200b", inline=False)
    embed.add_field(name="**`.ping`**",
                    value="Get the bot's latency.",
                    inline=True)
    embed.add_field(name="**`.status`**",
                    value="Get the Mojang API status.",
                    inline=True)
...
@slash.slash(name="status",
             description="Get the Mojang API status",
             options=[])
@client.command()
...
@slash.slash(name="clearcache", description="Clear the bot cache.", options=[])
@client.command()
...
async def invite(ctx):
    await ctx.send(
        "Here's the invite link: https://discord.com/api/oauth2/authorize?client_id=781349890944270366&permissions=2147503104&scope=bot%20applications.commands"
    )
